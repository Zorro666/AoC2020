...
Since it had been spoken before, the next number to speak is the difference between the turn number when it was last spoken (the previous turn, 4) and the turn number of the time it was most recently spoken before then (turn 1).
...
namespace Day15
{
    class Program
    {
        ...
        private static int RunGame(string line, int maxTurns)
        {
            var lastTurns = new int[maxTurns];
            var lastLastTurns = new int[maxTurns];
            var turn = 1;
            var lastNumber = 0;
            ...
            {
                var n = 0;
                if (lastLastTurns[lastNumber] > 0)
                {
                    n = lastTurns[lastNumber] - lastLastTurns[lastNumber];
                }
                lastLastTurns[n] = lastTurns[n];
                lastTurns[n] = turn;
                lastNumber = n;
        ...
        {
            return RunGame(lines[0], 2020);
        }

        public static int Part2(string[] lines)
        {
            return RunGame(lines[0], 30000000);
        }
