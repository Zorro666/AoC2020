...
namespace Day08
{
    class Program
    {
        ...
        private static int RunProgram(string[] lines, out int pc)
        {
            var acc = 0;
            pc = 0;
            var instructionsCount = lines.Length;
            var visited = new bool[instructionsCount];
            while (!visited[pc])
            {
                visited[pc] = true;
                var l = lines[pc].Trim();
                var tokens = l.Split();
                var instruction = tokens[0].Trim();
                ...
                {
                    throw new InvalidProgramException($"Unknown instruction {instruction} '{l}'");
                }
                if (pc == instructionsCount)
                {
                    return acc;
                }
                if ((pc < 0) || (pc > instructionsCount))
                {
                    return int.MinValue;
                }
            }
        ...
        {
            var instructionsCount = lines.Length;
            var acc = RunProgram(lines, out int pc);
            if ((pc < 0) || (pc >= instructionsCount))
            {
                throw new InvalidProgramException($"Invalid PC for Part1 {pc} {instructionsCount}");
            }
            return acc;
        }
        ...
        {
            var instructionsCount = lines.Length;
            var tried = new bool[instructionsCount];
            for (var i = 0; i < instructionsCount; ++i)
            {
                var instruction = lines[i].Trim().Split()[0].Trim();
                if (instruction == "nop")
                {
                    tried[i] = false;
                }
            ...
            while (true)
            {
                for (var i = nextInstructionToChange; i < instructionsCount; ++i)
                {
                    if (tried[i] == false)
                    {
                        tried[i] = true;
                        nextInstructionToChange = i;
                        break;
                    }
                }
                var instruction = lines[nextInstructionToChange].Trim().Split()[0].Trim();
                var savedInstruction = lines[nextInstructionToChange];
                if (instruction == "nop")
                {
                    lines[nextInstructionToChange] = lines[nextInstructionToChange].Replace("nop", "jmp");
                }
                else if (instruction == "jmp")
                {
                    lines[nextInstructionToChange] = lines[nextInstructionToChange].Replace("jmp", "nop");
                }
                else
                {
                    throw new InvalidProgramException($"Unknown instruction {instruction} {nextInstructionToChange} MAX: {instructionsCount}");
                }
                var acc = RunProgram(lines, out int pc);
                if (pc == instructionsCount)
                {
                ...
                lines[nextInstructionToChange] = savedInstruction;
