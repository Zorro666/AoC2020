    ...
    class Program
    {
        ...
        public static int Part1(string[] lines)
        {
            ...
            for (var a = 0; a < sAdapterCount; ++a)
            {
                ...
                else if ((currentVolt + 2) == adapter)
                {
                }
                else if ((currentVolt + 3) == adapter)
                {
                    ++threeVoltCount;
                }
                else
                {
                    throw new InvalidProgramException($"Can't find adapter {adapter} Volt {currentVolt}");
                }
                currentVolt = adapter;
            }
            ++threeVoltCount;
